...
class CleanSummary():
    ...
    def get_statistical_summary(self, variableType=None):
        ...
        if self.data.empty==True:
            ...
            # data points that are more than 3 standard deviations away from the mean in either direction are outliers
            ...
            for col in cols:
                if self.data[col].dtype in ['object','datetime64[ns]']:
                    ...
                    num_outliers = (abs(z_scores) > STDEV_THRESHOLD).sum()  
                    
                outliers_df = outliers_df.append({'variable': col, 
            ...
            for col in self.data.columns:
                if self.data[col].dtype not in ['object','datetime64[ns]']:
                    ...
                    normality_df = normality_df.append({
                        "variable":col,
                        "statistic":stat,
                        "pValue":p
                        }, ignore_index=True)
                    
            # if p_value > significance level, the distribution is normal

            SIGNIFICANCE_LEVEL = 0.05

            normality_df['normality'] = np.where(normality_df['pValue']> SIGNIFICANCE_LEVEL,True,False)
